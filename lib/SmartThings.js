// lib/SmartThings.js v1.1.4 (API Client Improvements)
'use strict';
...
class SmartThings {
    constructor(log, api, config) {
        ...
        this.tokenPath = path.join(this.api.user.persistPath(), 'smartthings_washer_token.json');
        ...
        // [v1.1.4 개선] API 재시도 로직에 지수 백오프 및 Jitter 적용
        axiosRetry(this.client, {
            retries: 3,
            retryDelay: (retryCount, error) => {
                this.log.info(`API 요청 재시도 (${retryCount}번째)... 응답코드: ${error.response?.status}`);
                // 1초, 2초, 4초 간격으로 재시도
                const delay = Math.pow(2, retryCount - 1) * 1000;
                // 약간의 무작위 시간(Jitter) 추가하여 동시 재시도 방지 (최대 500ms)
                const jitter = Math.random() * 500;
                return delay + jitter;
            },
            retryCondition: (error) => {
                const status = error.response?.status;
                // 네트워크 오류 또는 서버 측 오류(5xx), Rate Limit(429) 시 재시도
                return axiosRetry.isNetworkOrIdempotentRequestError(error) || status >= 500 || status === 429;
            }
        });

        this.setupInterceptors();
        // [v1.1.4 개선] 캐시 유지 시간을 5초에서 30초로 늘려 API 호출 빈도 감소
        this.cache = new LRUCache({ max: 100, ttl: 1000 * 30 });
    ...
    async getStatus(deviceId) {
        ...
        if (cachedData) {
            this.log.debug(`[${deviceId}] 상태를 캐시에서 불러옵니다.`);
            return cachedData;
        }
